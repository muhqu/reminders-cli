...
## 1. Ensure a compatible CLI

Run `../../scripts/ensure-compatible-cli.sh`, resolved from this skill's base directory. It
automatically installs or upgrades `muhqu/tap/reminders-cli` with Homebrew when the CLI is missing
or incompatible. On success, use the absolute binary path from the second output line for every
command below. On `error`, report the message and stop.

## 2. Work out the title and due date
...
## 5. Create it
```bash
"<reminders-binary>" add "Claude" "<title>" --due-date "<when>" --notes "<notes>" --format json
```
